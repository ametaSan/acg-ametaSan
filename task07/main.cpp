  ...
  for (unsigned int iw = 0; iw < img_width; ++iw) {
    for (unsigned int ih = 0; ih < img_height; ++ih) {
      ...
      for (int isample = 0; isample < nsample; ++isample) {
        ...
        float rad =  hit1_rad * hit0_brdf * std::max(0.f, hit0_refl.dot(hit0_normal)) / hit0_pdf;
      ...
      for (int isample = 0; isample < num_half_sample; ++isample) {
        ...
        float weight_light = hit0_pdf_light_sample/(hit0_pdf_brdf_sample + hit0_pdf_light_sample);
        float rad = weight_light * hit1_rad * hit0_brdf * std::max(0.f, hit0_refl.dot(hit0_normal)) / hit0_pdf_light_sample;
        img_mis[ih * img_width + iw] += rad/num_half_sample;
      }

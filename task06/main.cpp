...
auto sample_hemisphere(
  ...
  const float z = std::sqrt(1.0f - unirand.x());
...
void search_collision_in_bvh(
  ...
  if (bvhnodes[i_bvhnode].is_leaf()) { // this is leaf node
      ...
      search_collision_in_bvh(is_hit, hit_depth, hit_pos, hit_normal, bvhnodes[i_bvhnode].i_node_left, ray_org, ray_dir, tri2vtx, vtx2xyz, bvhnodes);
      search_collision_in_bvh(is_hit, hit_depth, hit_pos, hit_normal, bvhnodes[i_bvhnode].i_node_right, ray_org, ray_dir, tri2vtx, vtx2xyz, bvhnodes);
  }
}
      ...
      if (res) { // ambient occlusion computation
        ...
        for (unsigned int i_sample = 0; i_sample < num_sample_ao; ++i_sample) {
          ...
          if (!res1) { // if the ray doe not hit anything
              sum += (dir.dot(nrm) / pdf) / float(M_PI); // Problem 3: This is a bug. write some correct code (hint: use `dir.dot(nrm)`, `pdf`, `M_PI`).
          }
        }
